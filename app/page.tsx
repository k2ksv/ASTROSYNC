...
export default function HomePage() {
  const { sessions, isLoading, isSaving, error, saveSession, deleteSession } = useSessions();

  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-10">
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <StopwatchPanel onSave={saveSession} isSaving={isSaving} />
      ...
      <section className="space-y-4">
        ...
        <DashboardPanel sessions={sessions} isLoading={isLoading} onDelete={deleteSession} />
      </section>
    </main>
  );
}
